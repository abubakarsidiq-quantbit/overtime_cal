...
class TestEmployeeCheckinoutout(FrappeTestCase):
	def setUp(self):
		frappe.db.delete("Shift Type")
		frappe.db.delete("Shift Assignment")
		frappe.db.delete("Employee Checkinout")
	...
	def test_mark_attendance_and_link_log(self):
		...
		logs_count = frappe.db.count(
			"Employee Checkinout", {"name": ["in", log_names], "skip_auto_attendance": 1}
		)
		...
		log_names = [log.name for log in logs]
		logs_count = frappe.db.count(
			"Employee Checkinout", {"name": ["in", log_names], "attendance": attendance.name}
		)
	...
	def test_unlink_attendance_on_cancellation(self):
		...
		linked_logs = frappe.db.get_all("Employee Checkinout", {"attendance": attendance.name})
