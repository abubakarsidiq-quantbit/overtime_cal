...
class EmployeeCheckinout(Document):	
		...
		def validate_duplicate_log(self):
			doc = frappe.db.exists(
				"Employee Checkinout", {"employee": self.employee, "time": self.time, "name": ["!=", self.name]}
			)
			if doc:
				doc_link = frappe.get_desk_link("Employee Checkinout", doc)
				frappe.throw(
		...
		def fetch_shift(self):
			...
			if shift_actual_timings:
				if (
					...
					== "Strictly based on Log Type in Employee Checkinout"
...
def add_log_based_on_employee_field(
	...
	"""Finds the relevant Employee using the employee field value and creates a Employee Checkinout.
	...
	doc = frappe.new_doc("Employee Checkinout")
...
def mark_attendance_and_link_log(
	...
	"""Creates an attendance and links the attendance to the Employee Checkinout.
	Note: If attendance is already present for the given date, the logs are marked as skipped and no exception is thrown.

	:param logs: The List of 'Employee Checkinout'.
...
def calculate_working_hours(logs, check_in_out_type, working_hours_calc_type):
	...
	:param logs: The List of 'Employee Checkinout'.
	:param check_in_out_type: One of: 'Alternating entries as IN and OUT during the same shift', 'Strictly based on Log Type in Employee Checkinout'
	...
	elif check_in_out_type == "Strictly based on Log Type in Employee Checkinout":
		if working_hours_calc_type == "First Check-in and Last Check-out":
		...
		frappe.get_doc(
			{
				...
				"reference_doctype": "Employee Checkinout",
				"reference_name": name,
				"content": text,
			}
...
def skip_attendance_in_checkins(log_names):
	EmployeeCheckinout = frappe.qb.DocType("Employee Checkinout")
	(
		frappe.qb.update(EmployeeCheckinout)
		.set("skip_auto_attendance", 1)
		.where(EmployeeCheckinout.name.isin(log_names))
	).run()
